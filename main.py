...
g = 9.8
video = cv2.VideoWriter('output.mp4', cv2.VideoWriter_fourcc(*"MP4V"), 30, (960,960))
...
class Drone:
    def __init__(self, s, v, angle, angVel, shape:Rect, m:float):
        ...
        self.Fmin = -10     # 单桨叶能产生的最大反推力
    
    def autoControl(self, targetPoints):   # PID控制， 还没写
        F1, F2 = self.m*g/2, self.m*g/2
        F1, F2 = 1*F1, 1*F2
        return self.control(F1, F2)
...
class Env:
    ...
    def step(self):
        ...
        self.F1, self.F2 = d.calculateF(d.autoControl(self.target))  # 左右两边升力的大小
        ...
        Fsum += self.G   # 重力
        LimRandomF = 2   # 随机扰动的最大值
        Ra = R.random()*2*np.pi # 随机角度
        Rl = R.random()*LimRandomF/10 # 随机大小
    ...
    def __str__(self):
        d = self.drone
        return "t:%.3f\ts:%s\tv:%s\tangle:%s度\tangVel:%s度/s" % (self.t, str(d.s), str(d.v), str(d.angle/np.pi*180), str(d.angVel/np.pi*180))
...
def callback(self):
    ...
    obj.end = obj.start + 1/10*self.F1
    ...
    obj.end = obj.start + 1/10*self.F2
